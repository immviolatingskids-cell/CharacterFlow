...
function hydrate(){
  ...
  $('characterEdit').onclick=()=>{const character=state.character;const form=$('coreForm');form.name.value=character.name;form.age.value=character.identity.age;form.location.value=character.identity.location;form.occupation.value=character.identity.occupation;form.traits.value=(character.personality?.traits||[]).join(', ');form.interests.value=(character.interests||[]).join(', ');$('coreReferenceStatus').textContent=character.references?.length?`${character.references.length} reference${character.references.length===1?'':'s'} attached`:'No references attached';setDrawer('core',true)};
