...
export function normalizeProject(project={},state={}){
  ...
  const inspirationRefs=(Array.isArray(project.inspirationRefs)?project.inspirationRefs:[]).map((reference,index)=>({id:String(reference.id||`inspiration_${project.id||'project'}_${index+1}`),createdAt:reference.createdAt||project.createdAt||timestamp(),title:String(reference.title||'Creative Direction'),concepts:unique(reference.concepts).map(String),weights:{...(reference.weights||{})},selected:{...(reference.selected||{})},seed:reference.seed==null?null:String(reference.seed)}));
  return {id:project.id||makeId(),name:String(project.name||'Untitled Project'),brief:String(project.brief||''),status:project.status==='archived'?'archived':'active',characterIds,takeIds,direction:{notes:String(project.direction?.notes||''),sceneIntent:String(project.direction?.sceneIntent||''),styleIntent:String(project.direction?.styleIntent||'')},inspirationRefs,createdAt:project.createdAt||timestamp(),updatedAt:project.updatedAt||project.createdAt||timestamp()};
}
...
export function addProjectInspiration(state,projectId,reference={},options={}){const project=(state.projects||[]).find(item=>item.id===projectId);if(!project)return state;const now=options.now||timestamp(),id=reference.id||options.id||`inspiration_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;return updateProject(state,projectId,{inspirationRefs:[...(project.inspirationRefs||[]),{...clone(reference),id,createdAt:reference.createdAt||now}]},{...options,now})}
