...
export function buildInspirationFromTakeSignals(inspirationState,takes=[],signals=[]){const takeById=new Map(takes.map(take=>[take.id,withTakeSemantics(take)])),selected=signals.filter(signal=>signal?.takeId&&TAKE_SIGNAL_DOMAINS.includes(signal.domain)&&takeById.has(signal.takeId));const concepts=[];const provenance=new Map();for(const signal of selected){const take=takeById.get(signal.takeId);for(const concept of takeSemanticsToConcepts(take,{domains:[signal.domain]})){const key=`${concept.domain}:${concept.id}`;if(!provenance.has(key)){provenance.set(key,{...concept,provenance:[concept.provenance]});concepts.push(provenance.get(key));}else provenance.get(key).provenance.push(concept.provenance);}}const current=normalizeInspirationState(inspirationState);return {...current,mode:'create',rawIdea:concepts.map(concept=>concept.label).join(', '),concepts,direction:null,source:{type:'takes',takeIds:[...new Set(selected.map(signal=>signal.takeId))],signals:clone(selected)}};}
