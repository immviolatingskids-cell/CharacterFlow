...
function candidatesFor(category,mix,state,registry,seed) {
  ...
  const occupation=normalizeOccupation(state.character?.identity?.occupation);
  for(const influence of mix){const pack=registry[influence.packId];const domains=[category,...(STYLE_DOMAINS[category]||[])];const domainAtoms=[...new Map(domains.flatMap(domain=>pack.atoms[domain]||[]).map(atom=>[atom.id,atom])).values()];const relationship=occupation&&relationshipsForStylePack(pack.id).find(item=>item.occupationId===occupation.id);for(const atom of domainAtoms){const adjustment=adjustmentFor(atom,tokens);const occupationMultiplier=relationship?1+Math.min(.12,relationship.weight*.12):1;const raw=influence.normalizedWeight*atom.weight;const adjusted=raw*adjustment.multiplier*occupationMultiplier;const entry=byId.get(atom.id)||{id:atom.id,value:clone(atom.value),conflicts:[...(atom.conflicts||[])],support:0,adjustedSupport:0,contributions:[],adjustments:[]};entry.support+=raw;entry.adjustedSupport+=adjusted;entry.contributions.push({packId:pack.id,packName:pack.name,normalizedPackWeight:influence.normalizedWeight,atomWeight:atom.weight,rawSupport:round(raw),compatibilityMultiplier:adjustment.multiplier,occupationMultiplier:round(occupationMultiplier),adjustedSupport:round(adjusted)});entry.adjustments.push(...adjustment.adjustments.map(item=>({packId:pack.id,...item})));entry.conflicts=[...new Set([...entry.conflicts,...(atom.conflicts||[])])];byId.set(atom.id,entry);}}
  return [...byId.values()].map(candidate=>{const seedFactor=.95+seededUnit(seed,category,candidate.id)*.1;const score=candidate.adjustedSupport*seedFactor;return {...candidate,support:round(candidate.support+(candidate.contributions.length>1?.01:0)),adjustedSupport:round(candidate.adjustedSupport),seedFactor:round(seedFactor),score:round(score)};}).sort((a,b)=>b.score-a.score||a.id.localeCompare(b.id));
}
