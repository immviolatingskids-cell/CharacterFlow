...
function candidatesFor(category,mix,state,registry,seed) {
  ...
  const strength=Math.min(1,Math.max(0,Number.isFinite(Number(state.styleMix?.strength))?Number(state.styleMix?.strength):1));
  ...
  return [...byId.values()].map(candidate=>{const seedFactor=.95+seededUnit(seed,category,candidate.id)*.1;const score=strength===0?0:Math.pow(Math.max(candidate.adjustedSupport,0),strength)*seedFactor;return {...candidate,support:round(candidate.support),adjustedSupport:round(candidate.adjustedSupport),seedFactor:round(seedFactor),score:round(score)};}).sort((a,b)=>b.score-a.score||a.id.localeCompare(b.id));
}
