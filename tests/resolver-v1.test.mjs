...
test('style strength lowers Style authority without flattening candidate ranking',()=>{const state=withMix([['tech-girlie',70],['streetwear',30]]);const low=resolveStudioState({...state,styleMix:{...state.styleMix,strength:.25}},{seed:'strength'});const high=resolveStudioState({...state,styleMix:{...state.styleMix,strength:1}},{seed:'strength'});const lowScores=low.provenance.categories.wardrobe.candidates.map(item=>item.id);const highScores=high.provenance.categories.wardrobe.candidates.map(item=>item.id);assert.deepEqual(lowScores,highScores);assert.ok(low.provenance.categories.wardrobe.styleAuthority<high.provenance.categories.wardrobe.styleAuthority);assert.equal(low.provenance.categories.wardrobe.selection.authority<high.provenance.categories.wardrobe.selection.authority,true);const zero=resolveStudioState({...state,styleMix:{...state.styleMix,strength:0}},{seed:'strength'});assert.equal(zero.resolved.wardrobe,null);assert.equal(zero.styleMix.strength,0)});
