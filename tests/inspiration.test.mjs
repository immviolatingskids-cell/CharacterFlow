...
test('From Takes keeps equal IDs in different domains independent and excludes missing Take sources',()=>{const semantic={schemaVersion:1,style:[],setting:[],activity:[{id:'candid',label:'Candid'}],wardrobe:[],camera:[{id:'candid',label:'Candid'},{id:'35mm',label:'35mm'}],lighting:[],mood:[],tags:[]},takes=[{id:'a',characterId:'a',semantic},{id:'b',characterId:'b',semantic}],signals=[{takeId:'a',domain:'camera'},{takeId:'b',domain:'camera'},{takeId:'a',domain:'activity'},{takeId:'missing',domain:'camera'}],result=buildInspirationFromTakeSignals(createDefaultInspirationState(),takes,signals);assert.equal(result.concepts.filter(concept=>concept.id==='35mm').length,1);assert.equal(result.concepts.find(concept=>concept.domain==='camera'&&concept.id==='35mm').provenance.length,2);assert.equal(result.concepts.filter(concept=>concept.id==='candid').length,2);assert.deepEqual(result.source.takeIds,['a','b']);assert.equal(result.source.signals.length,3);});
