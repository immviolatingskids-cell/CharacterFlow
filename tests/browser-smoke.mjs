...
try{
  ...
  assert.equal(exceptions.length,0);assert.match(result.character,/Nova/);assert.ok(result.activePacks.includes('Tech Girlie'));assert.equal(result.scene,'Café');assert.ok(result.takes>=1);assert.equal(result.forge,'idle');assert.equal(result.focusMode,true);assert.match(result.library.workspace,/Library/);assert.match(result.library.category,/Interests/);assert.equal(result.library.families,1);assert.equal(result.library.detail,'Gaming');assert.equal(result.library.query,'Speedrunning');assert.equal(result.library.starter,'LIVE CONTENT');assert.equal(result.library.gemini,'Prepare review');assert.equal(result.laptopOverflow,true);assert.equal(result.mobileOverflow,true);assert.equal(result.reducedMotion,true);assert.equal(result.stageButtonFont,'14px');assert.match(result.compilerStatus,/MockCompiler/);assert.equal(result.screenshot,true);
