...
try{
  ...
  await evaluate("document.querySelector('.sidenav [data-workspace=\"account\"]').click()");await wait(250);
  await evaluate("document.getElementById('accountLocal')?.click()");await wait(100);
  result.account=await evaluate("(()=>({title:document.querySelector('.account-title h1')?.textContent,stats:document.querySelectorAll('.account-stats article').length,quickActions:document.querySelectorAll('[data-account-go]').length,semanticIcons:document.querySelectorAll('.account-dashboard .pfi').length,localMode:document.querySelector('.account-mode')?.textContent,recent:document.querySelectorAll('.creation-card').length}))()");
  await command('Emulation.setDeviceMetricsOverride',{width:741,height:600,deviceScaleFactor:1,mobile:false});await wait(150);
  result.laptopOverflow=await evaluate("document.documentElement.scrollWidth<=window.innerWidth");result.accountLaptopOverflow=result.laptopOverflow;
  await command('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});await wait(150);
  result.mobileOverflow=await evaluate("document.documentElement.scrollWidth<=window.innerWidth");result.accountMobileOverflow=result.mobileOverflow;
  ...
  assert.equal(exceptions.length,0);assert.match(result.character,/Nova/);assert.ok(result.activePacks.includes('Tech Girlie'));assert.equal(result.scene,'Café');assert.ok(result.takes>=1);assert.equal(result.forge,'idle');assert.equal(result.focusMode,true);assert.match(result.library.workspace,/Library/);assert.match(result.library.category,/Interests/);assert.equal(result.library.families,1);assert.equal(result.library.detail,'Gaming');assert.equal(result.library.query,'Speedrunning');assert.equal(result.library.starter,'LIVE CONTENT');assert.equal(result.library.gemini,'Prepare review');assert.equal(result.account.title,'Account Hub');assert.equal(result.account.stats,6);assert.ok(result.account.quickActions>=5);assert.ok(result.account.semanticIcons>=12);assert.match(result.account.localMode,/Local workspace/);assert.ok(result.account.recent>=1);assert.equal(result.accountLaptopOverflow,true);assert.equal(result.accountMobileOverflow,true);assert.equal(result.reducedMotion,true);assert.equal(result.stageButtonFont,'14px');assert.match(result.compilerStatus,/MockCompiler/);assert.equal(result.screenshot,true);
