...
try{
  ...
  await evaluate("localStorage.clear();document.getElementById('surpriseBtn').click()");await wait(1300);
  ...
  const result=await evaluate("(()=>{document.getElementById('inspectBtn').click();const output=document.getElementById('drawerInspectorOutput').textContent||document.getElementById('inspectorOutput').textContent;const parsed=JSON.parse(output);return {title:document.title,character:document.getElementById('studioTitle').textContent,activePacks:[...document.querySelectorAll('.mix-card b')].map(node=>node.textContent),scene:document.getElementById('sceneText').textContent,takes:document.querySelectorAll('[data-take]').length,forge:document.querySelector('.forge-orbital').dataset.forgeState,inspectorOpen:document.getElementById('inspectorDrawer').classList.contains('open'),resolverVersion:parsed.resolver.version,normalizedMix:parsed.normalizedMix,resolvedCategories:Object.keys(parsed.resolved),provenanceCategories:Object.keys(parsed.provenance)}})()");
  ...
  result.stageButtonFont=await evaluate("getComputedStyle(document.getElementById('newTakeBtn')).fontSize");
