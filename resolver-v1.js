...
export function normalizeStyleMix(influences = [], registry = STYLE_PACKS) {
  const valid = influences.filter(item => registry[item.packId] && Number(item.weight) > 0).map(item => ({packId:item.packId,weight:Number(item.weight)}));
  const total = valid.reduce((sum,item) => sum + item.weight, 0);
  return valid.map(item => ({...item,normalizedWeight:round(item.weight / total)}));
}
...
function candidatesFor(category,mix,state,registry,seed) {
  ...
  const strength=Number(state.styleMix?.strength??1);
  for(const influence of mix){const pack=registry[influence.packId];for(const atom of pack.atoms[category]||[]){const adjustment=adjustmentFor(atom,tokens);const raw=influence.normalizedWeight*atom.weight*strength;const adjusted=raw*adjustment.multiplier;const entry=byId.get(atom.id)||{id:atom.id,value:clone(atom.value),conflicts:[...(atom.conflicts||[])],support:0,adjustedSupport:0,contributions:[],adjustments:[]};entry.support+=raw;entry.adjustedSupport+=adjusted;entry.contributions.push({packId:pack.id,packName:pack.name,normalizedPackWeight:influence.normalizedWeight,atomWeight:atom.weight,styleStrength:strength,rawSupport:round(raw),compatibilityMultiplier:adjustment.multiplier,adjustedSupport:round(adjusted)});entry.adjustments.push(...adjustment.adjustments.map(item=>({packId:pack.id,...item})));entry.conflicts=[...new Set([...entry.conflicts,...(atom.conflicts||[])])];byId.set(atom.id,entry);}}
  return [...byId.values()].map(candidate=>{const seedFactor=.85+seededUnit(seed,category,candidate.id)*.3;return {...candidate,support:round(candidate.support),adjustedSupport:round(candidate.adjustedSupport),seedFactor:round(seedFactor),score:round(candidate.adjustedSupport*seedFactor)};}).sort((a,b)=>b.score-a.score||a.id.localeCompare(b.id));
}
...
export function resolveStyleState(state,seed='promptforge-default',registry=STYLE_PACKS) {
  ...
  for(const category of RESOLVER_CATEGORIES){const override=manualValue(state,category);const candidates=candidatesFor(category,normalizedMix,state,registry,seed);if(override){resolved[category]=override.value;categories[category]={selection:{id:null,value:clone(override.value),score:null,source:override.source},candidates};continue;}const selected=candidates.find(candidate=>candidate.score>0&&!candidate.conflicts.some(id=>selectedIds.has(id)))||null;resolved[category]=selected?clone(selected.value):null;if(selected)selectedIds.add(selected.id);categories[category]={selection:selected?{id:selected.id,value:clone(selected.value),score:selected.score,source:'style-mix'}:null,candidates};}
  if(JSON.stringify(state.character)!==characterBefore)throw new Error('Resolver attempted to mutate Character Core');
  return {resolver:{name:'PromptForge Resolver',version:1,seed:String(seed),strategy:'fnv1a-plus-seeded-jitter'},character:clone(state.character),styleMix:{strength:Number(state.styleMix?.strength??1),normalizedInfluences:normalizedMix},context:{scene:clone(state.scene||null),activeContext:clone(state.interestState?.activeContext||[]),wardrobe:clone(state.wardrobe||null),visualSettings:clone(state.visualSettings||null)},resolved,provenance:{categories}};
}
